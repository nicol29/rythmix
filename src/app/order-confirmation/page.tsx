...
export default async function OrderConfirmation({ 
  ...
  const itemsPurchased = await Seller_Payouts.find({ transferGroup: paymentIntent.transfer_group })
  .populate({
    path: 'productId', // This populates each 'productId' in the 'items' array
    select: 'assets title' // Replace 'name price' with the actual fields you want from the Product documents
  });
  ...
  const returnTotal = () => {
    return itemsPurchased.reduce((acc, item) => acc + (item.totalAmount / 100), 0);
  }
                  ...
                  <div className="flex flex-col gap-4">
                    {itemsPurchased.map((item, index) => (
                      <>
                        ...
                        {(itemsPurchased.length - 1) !== index && <div className="w-full h-[1px] bg-neutral-700"></div>}
                      </>
                    ))}
                  </div>
                </div>
              </div>
              <div className="bg-neutral-850 px-12 py-6 drop-shadow-xl rounded-r">
                    ...
                    <div key={item._id.toString()} className="flex items-center gap-2">
                      ...
                      <div className="truncate overflow-hidden flex-1">
                        ...
                        <div className="flex justify-between mt-2">
                          ...
                          <span className="font-semibold">€ {item.totalAmount / 100}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex justify-between border-tpt-2 mt-4 mb-8 font-bold">
                  <span>Total ({itemsPurchased.length} items)</span>
